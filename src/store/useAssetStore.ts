...
const BUILDING_CONFIG: Record<string, BuildingConfig> = {
  ...
  factory: {
    ...
    incomeRate: 150,
    dailyPowerDrip: 4,
    institutionType: "factory_supply",
  },
  // ── Public T4 ───────────────────────────────────────────────────────────
  hospital: {
    cost: 8000,
...
function dbRowToAsset(row: Record<string, any>): Asset {
  return {
    ...
    institutionCategory: (row.institution_category ??
      null) as InstitutionCategory | null,
...
function buildEmptySession(
  ...
  const slots: BuildMaterialSlot[] = slotDefs.map((s) => ({
    ...
    itemId: s.itemId,
    nameFa:
      BUILD_MATERIALS.find((m) => m.itemId === s.itemId)?.nameFa ?? s.itemId,
...
function recomputeSession(session: AdvancedBuildSession): AdvancedBuildSession {
  ...
  const effectivePowerRatio =
    gatherCount > 0 ? Math.round((sumRatio / gatherCount) * 1000) / 1000 : 1.0;
...
export const useAssetStore = create<AssetState>()((set, get) => ({
        ...
        (payload) => {
          if (
            ...
            set((state) => ({
              assets: {
                ...state.assets,
                [updated.id]: { ...state.assets[updated.id], ...updated },
              },
            }));
  ...
  buildAssetFast: async ({
    ...
    const category: InstitutionCategory | null =
  ...
  addMaterialToSession: async ({
    ...
    try {
      ...
      const updated = recomputeSession({
        ...activeSession,
        slots: updatedSlots,
      });
  ...
  confirmAdvancedBuild: async () => {
    ...
    const neighborhoodId = (activeSession as any).__neighborhoodId as
      | string
      | null
      | undefined;
    const costMultiplier = useNeighborhoodStore
    ...
    try {
      ...
      if (!data?.success) {
        console.warn(
          "[AssetStore] confirmAdvancedBuild RPC error:",
          data?.error,
        );
        return null;
      }
  ...
  fillWarehouse: async (industrialAssetId, commercialAssetId) => {
    ...
    const powerReward =
      INSTITUTION_DEFINITIONS[indAsset.institutionType ?? ""]
        ?.providerGainPower ?? 2;
    ...
    try {
      ...
      set((state) => ({
        assets: {
          ...state.assets,
          [commercialAssetId]: {
            ...state.assets[commercialAssetId],
  ...
  fetchNearbyShopItems: async (latitude, longitude) => {
    ...
    try {
      ...
      const { data: nearbyAssets } = await supabase
        .from("assets")
        .select(
          "id, owner_id, type, institution_category, owner:profiles(username)",
        )
  ...
  fetchListings: async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("asset_listings")
        .select(
          `*, asset:assets!asset_listings_asset_id_fkey(*), seller:profiles!asset_listings_seller_id_fkey(username)`,
        )
        .eq("status", "active")
